...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  try {
    ...
    thisProduct = await response.json();
  } catch (error: any) {
              ...
              <div className="data w-full lg:pr-8 pr-0 justify-center flex items-center max-lg:pb-10 xl:my-2 lg:my-5 my-0">
                <div className="data w-full max-w-xl">
                  ...
                  <ul className="grid gap-y-4 mb-8">
                    {product.essentialCharacteristics.map((carac, index) => (
                      <li key={`${product.code}-${index}`} className="flex items-center gap-3">
                        ...
                        <span className="font-normal text-base text-gray-900">
                          {carac.name} : {carac.featureValues[0].value}
                        </span>
                      </li>
                    ))}
                  </ul>
